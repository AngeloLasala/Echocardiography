"""
Main file to train the PLAX regression model with sigma as learnable parameter
"""
import os
import argparse
...
def train_one_epoch(training_loader, model, loss, optimizer, device, tb_writer = None):
    ...
    for i, (inputs, labels) in enumerate(training_loader):
        ...
        # ## plt the image and the coordinate and the flipped coordinate
        # # detaching the tensor
        # inputs = inputs.detach().cpu().numpy()
        # labels = labels.detach().cpu().numpy()
        # plt.imshow(inputs[4,0,:,:], cmap='gray')
        # plt.scatter(labels[4,0]*256, labels[4,1]*256, c='r')
        # plt.show()
        
        optimizer.zero_grad()                           # Zero your gradients for every batch!
        outputs, targets = model(inputs, labels)                         # Make predictions for this batch
        loss = loss_fn(outputs.float(), targets.float()) # Compute the loss and its gradientsù
        loss.backward()
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            for i, vdata in enumerate(validation_loader):
                ...
                voutputs, vtargets = model(vinputs, vlabels)
                vloss = loss_fn(voutputs, vtargets).item()
                running_vloss += vloss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target', type=str, default='heatmaps_sigma', help=' heatmaps_sigma')
